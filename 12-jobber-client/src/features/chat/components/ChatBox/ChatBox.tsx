import { ChangeEvent, type FC, type FormEvent, type ReactElement, RefObject, useRef, useState } from "react";
...
import type { IChatBoxProps, IConversationDocument, IMessageDocument } from "../../interfaces/chat.interface";
...
const ChatBox: FC<IChatBoxProps> = ({ seller, buyer, gigId, onClose }): ReactElement => {
  ...
  if (isConversationSuccess && conversationData && conversationData.conversations && conversationData.conversations.length) {
            ...
            <div className="my-2 flex h-[280px] flex-col overflow-y-scroll px-4 md:h-[72%]" ref={scrollRef}>
              {chatMessages.map((msg: IMessageDocument) => (
                ...
                >
                  ...
                  />
                  <p
                    className={`ml-2 max-w-[200px] rounded-[10px] bg-[#e4e6eb] px-4 py-2 text-start text-sm font-normal md:max-w-[220px]  text-black ${
