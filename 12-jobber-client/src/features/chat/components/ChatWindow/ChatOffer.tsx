...
import { createSearchParams, type NavigateFunction, useNavigate } from "react-router-dom";
import type { IOffer } from "src/features/order/interfaces/order.interface";
import Button from "src/shared/button/Button";
import { showErrorToast } from "src/shared/utils/utils.service";

import type { IChatMessageProps } from "../../interfaces/chat.interface";
    ...
    <div className="z-1 border-grey mt-2 flex h-72 max-w-xl flex-col overflow-hidden rounded border">
      <div className="w-full">
        ...
        <div className="relative top-[5%] mr-3 flex flex-row justify-end gap-4">
          <Button
            className={`rounded px-6 py-3 text-center text-sm font-bold text-white focus:outline-none md:px-4 md:py-2 md:text-base ${messageOffer.accepted || messageOffer.cancelled ? "cursor-not-allowed bg-red-200 hover:bg-red-200" : "bg-red-500 hover:bg-red-400"}`}
          ...
          {seller && seller._id !== message.sellerId && (
            <Button
              className={`rounded px-6 py-3 text-center text-sm font-bold text-white focus:outline-none md:px-4 md:py-2 md:text-base ${messageOffer.accepted || messageOffer.cancelled ? "cursor-not-allowed bg-sky-200 hover:bg-sky-200" : "bg-sky-500 hover:bg-sky-400"}`}
              disabled={messageOffer.accepted || messageOffer.cancelled}
              label="Accept Offer"
