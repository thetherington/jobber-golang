...
import { type ChangeEvent, type FC, type FormEvent, type ReactElement, useEffect, useRef, useState } from "react";
...
import type { IChatWindowProps, IMessageDocument } from "../../interfaces/chat.interface";
...
const NONE_EXISTING_ID = "662007bcf28c0f43993d1666";

const ChatWindow: FC<IChatWindowProps> = ({ chatMessages, isLoading, setSkip }): ReactElement => {
  const { username } = useParams<string>();
  const { online, getLoggedInUsers } = useSocket();
  const dispatch = useAppDispatch();

  const authUser = useAppSelector((state: IReduxState) => state.authUser);
  const seller = useAppSelector((state: IReduxState) => state.seller);

  const [userOnline, setUserOnline] = useState<boolean>(MESSAGE_STATUS.IS_LOADING);
  ...
  const { data } = useGetGigByIdQuery(singleMessageRef.current ? `${singleMessageRef.current.gigId}` : NONE_EXISTING_ID);
  ...
  //   if (find(data, (name: string) => name === firstLetterUppercase(`${username}`))) {
  //     setUserOnline(true);
  //   } else {
  ...
  useEffect(() => {
    if (find(online, (name: string) => name === firstLetterUppercase(`${username}`))) {
      setUserOnline(true);
    } else {
      console.log("not found");
      setUserOnline(false);
