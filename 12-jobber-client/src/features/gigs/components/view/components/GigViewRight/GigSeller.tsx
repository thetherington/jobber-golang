import { type FC, lazy, LazyExoticComponent, type ReactElement, Suspense, useState } from "react";
import { FaArrowRight } from "react-icons/fa";
import { Link } from "react-router-dom";
import type { IChatBoxProps, IChatBuyerProps, IChatSellerProps } from "src/features/chat/interfaces/chat.interface";
import { useGig } from "src/features/gigs/context/GigContext";
import type { ILanguage } from "src/features/sellers/interfaces/seller.interface";
...
const ChatBox: LazyExoticComponent<FC<IChatBoxProps>> = lazy(() => import("src/features/chat/components/ChatBox/ChatBox"));

const GigSeller: FC = (): ReactElement => {
  const { gig, seller } = useGig();
        ...
        <div className="mb-0 px-4 pt-2">
          ...
          <div className="ml-15 mb-2 flex w-full py-1">
            <Button
              ...
              className={`text-md flex w-full  justify-between rounded bg-sky-500 px-8 py-2 font-bold text-white focus:outline-none ${authUser.username === gig.username ? "opacity-20 cursor-not-allowed" : "hover:bg-sky-400 cursor-pointer"}`}
              label={
                <>
