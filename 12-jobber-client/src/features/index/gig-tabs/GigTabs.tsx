...
const GigTabs: FC = (): ReactElement => {
  ...
  let categoryGig: ISellerGig[] = [];

  if (isSuccess) {
    categoryGig = data.gigs as ISellerGig[];
  }

  useEffect(() => {
    setActiveTab("Graphics & Design");
  }, []);
        ...
        <div className="mt-4 h-full overflow-hidden border p-6">
          {categoryGig.length > 0 ? (
            <>
              ...
              <TopGigsView gigs={categoryGig} width="w-72" type="index" />
            </>
          ) : (
            <div className="flex h-96 items-center justify-center text-lg">Information not available</div>
          )}
        </div>
      </div>
    </div>
