...
const Checkout: FC = (): ReactElement => {
  // memoize this method so that loadStripe doesn't get called if a goes back to the page
  ...
  const createBuyerOrderIntent = async (): Promise<void> => {
    try {
      const response: IResponse = await createOrderIntent({ price: offer.price, buyerId: buyer._id! }).unwrap();
