...
// import type { IOrderDocument } from "src/features/order/interfaces/order.interface";
...
import type { SellerContextType } from "../../interfaces/seller.interface";
...
const ManageOrders: FC = (): ReactElement => {
  ...
  useEffect(() => {
    ...
    const index = findIndex(ordersRef, ["orderId", orderDocument.orderId]);

    if (index > -1) {
      ordersRef.splice(index, 1, orderDocument);
    }
  }, [orderDocument]); // eslint-disable-line
        ...
        <div className="p-0">
          <ul className="flex w-full cursor-pointer list-none flex-col flex-wrap rounded-[2px] sm:flex-none sm:flex-row">
            <li className="inline-block py-3 uppercase" onClick={() => setType(SELLER_GIG_STATUS.ACTIVE)}>
              <a
                href="#activeorders"
                className={`px-4 py-3 text-xs text-[#555555] no-underline sm:text-sm md:text-base ${type === SELLER_GIG_STATUS.ACTIVE ? "pb-[15px] outline outline-1 outline-[#e8e8e8] sm:rounded-t-lg" : ""}`}
              >
                Active
