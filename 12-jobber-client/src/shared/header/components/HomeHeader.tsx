...
import { type FC, lazy, type LazyExoticComponent, type ReactElement, Suspense, useEffect, useRef, useState } from "react";
...
// import type { IMessageDocument } from "src/features/chat/interfaces/chat.interface";
import type { IOrderNotifcation } from "src/features/order/interfaces/order.interface";
...
import type { IBannerProps, IResponse } from "src/shared/shared.interface";
...
const Banner: LazyExoticComponent<FC<IBannerProps>> = lazy(() => import("src/shared/banner/Banner"));
...
const HomeHeader: FC<IHomeHeaderProps> = ({ showCategoryContainer }): ReactElement => {
  ...
  useEffect(() => {
    if (isReady && authUser.username !== null) {
      setLoggedInUser(authUser.username);
      setUsername(authUser.username);
    }
  }, [isReady, authUser.username]); // eslint-disable-line

  useEffect(() => {
    const username = find(online, (name: string) => name === authUser.username);
  ...
  useEffect(() => {
    ...
    if (socketMessage.receiverUsername === `${authUser.username}` && !socketMessage.isRead) {
      dispatch(updateNotification({ hasUnreadMessage: true }));
    }
  }, [socketMessage]); // eslint-disable-line

  useEffect(() => {
    if (socketNotification === undefined) {
      return;
    }

    if (socketNotification.userTo === `${authUser.username}` && !socketNotification.isRead) {
      dispatch(updateNotification({ hasUnreadNotification: true }));
    }
  }, [socketNotification]); // eslint-disable-line
  ...
  }, [isSuccess, authUser.username, data?.notifications, dispatch]); // eslint-disable-line
                ...
                <div className="text-[#74767e] lg:pr-4">
                  <ul className="flex text-base font-medium">
                    ...
                    <li className="relative z-50 flex cursor-pointer items-center">
                      ...
                      >
                        <div className="absolute right-0 mt-5 w-96">
                          <OrderDropdown buyer={buyer} setIsDropdownOpen={setIsOrderDropdownOpen} />
                        </div>
                      </Transition>
                    </li>
                        ...
                        label={
                          <>
                            ...
                            {authUsername === authUser.username && (
                              <span className="absolute bottom-0 left-8 h-2.5 w-2.5 self-center rounded-full border-2 border-white bg-green-400"></span>
                            )}
