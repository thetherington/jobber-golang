import { ChangeEvent, type FC, type ReactElement, useState } from "react";
import type { IExtendedDateModalProps, IExtendedDelivery } from "src/features/order/interfaces/order.interface";
...
const ExtendDateModal: FC<IExtendedDateModalProps> = ({ order, onClose }): ReactElement => {
  ...
  const requestExtension = async (): Promise<void> => {
    try {
      const oldDate: Date = new Date(order.offer.oldDeliveryDate);
        ...
        <div className="relative bottom-auto left-auto right-auto top-auto max-h-[90vh] min-w-[350px] bg-white p-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              className={`cursor-pointer rounded px-6 py-3 text-center text-sm font-bold text-white focus:outline-none md:px-4 md:py-2 md:text-base ${!reason || !deliveryDate ? "cursor-not-allowed bg-sky-200" : "bg-sky-500 hover:bg-sky-400"}`}
              label="Send Request"
