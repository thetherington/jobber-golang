...
export const applicationLogout = (dispatch: AppDispatch, navigate: NavigateFunction) => {
  const loggedInUsername: string = getDataFromSessionStorage("loggedInuser");

  // clears the state to be empty via the root reducer
  dispatch(logout({}));

  // calls the backend to remove the user from the redis cache lookup
  if (loggedInUsername) {
  ...
  // calls the backend to remove the session from the backend session
  dispatch(authApi.endpoints.logout.initiate());

  // udpates local storage and blanks out keys
